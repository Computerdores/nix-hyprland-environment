...
let
    pidFilePath = "/tmp/idle-inhibit.pid";
in {
    environment.systemPackages = [ (pkgs.writeShellScriptBin "sleep-inhibit" ''
    ...
    '' )];
    systemd.user.services.clean-sleep-inhibit-pid-file = {
        description = "Delete sleep-inhibit pid file on graphical session start";
        ...
        serviceConfig = {
            Type = "oneshot";
            ExecStart = "${pkgs.coreutils}/bin/rm ${pidFilePath}";
        };
    };
}
